...
def require_admin(request: Request, session: Session, *, allow_password_change: bool = False) -> tuple[AdminCredential, AdminSession]:
    if not settings.require_admin_auth:
        ...
        if credential is None:
            ...
            credential = session.get(AdminCredential, 1)
        if credential is None:
            raise HTTPException(status_code=503, detail="管理员账户尚未初始化")
        # Callers only need the session for CSRF validation, which is also
        # disabled in this explicitly opted-in compatibility mode.
        return credential, AdminSession(csrf_token="")
