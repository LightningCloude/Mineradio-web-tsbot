...
from .beat_cache import get_cached_analysis, normalize_song_name, store_cached_analysis
from .bandwidth import copy_chunks_with_rate_limit
from .playback_contracts import build_ws_progress_payload, resolve_qqmusic_cover_url
from .auth import (
...
app.add_middleware(
    ...
    allow_credentials=bool(settings.get_cors_origins()),
...
_BILIBILI_DOWNLOAD_RATE_BPS = max(1, int(settings.bilibili_download_rate_kbps)) * 1024
_BILIBILI_DOWNLOAD_CONCURRENCY = max(1, int(settings.bilibili_download_concurrency))
_bilibili_download_semaphore = asyncio.Semaphore(_BILIBILI_DOWNLOAD_CONCURRENCY)
...
class BeatAnalysisCacheRequest(BaseModel):
    name: str
...
def _download_bilibili_audio_sync(video_id: str) -> tuple[str, dict | None]:
        ...
        with httpx.Client(timeout=60.0, follow_redirects=True, headers=headers) as client:
            with client.stream("GET", download_url) as resp:
                ...
                with open(tmp_path, "wb") as fh:
                    copy_chunks_with_rate_limit(
                        resp.iter_bytes(chunk_size=64 * 1024),
                        fh.write,
                        _BILIBILI_DOWNLOAD_RATE_BPS,
                    )
    ...
    options = {
        ...
        "retries": 2,
        "fragment_retries": 2,
        "ratelimit": _BILIBILI_DOWNLOAD_RATE_BPS,
        "http_headers": _BILIBILI_DEFAULT_HEADERS,
    }
    try:
        with yt_dlp.YoutubeDL(options) as ydl:
...
async def _download_bilibili_audio(video_id: str) -> tuple[str, dict | None]:
    ...
    async with lock:
        ...
        async with _bilibili_download_semaphore:
            # A different waiter may have populated the cache while this task
            # was queued behind the global bandwidth gate.
            cached_path = _find_cached_bilibili_audio(normalized_video_id)
            if cached_path:
                return cached_path, None
            try:
...
def _attach_lrc_translations(
    lyrics: list[LyricLine], translated_lrc: str
) -> list[LyricLine]:
    """Attach native provider translations without changing the lyric timeline."""
    translations = [line for line in _parse_lrc_to_lines(translated_lrc) if line.text]
    if not lyrics or not translations:
        return lyrics

    aligned = align_native_translations(
        [(line.time, line.text) for line in lyrics],
        [(line.time, line.text) for line in translations],
    )
    for lyric, translation in zip(lyrics, aligned):
        lyric.translation = translation

    return lyrics


def _lyrics_with_translation(lrc: str, translated_lrc: str = "") -> list[LyricLine]:
    return _attach_lrc_translations(_parse_lrc_to_lines(lrc), translated_lrc)
...
async def lyrics(queue_item_id: int) -> LyricsResponse:
    ...
    if track_id.startswith("netease:"):
        ...
        try:
            ...
            translated = data.get("trans", "") if data else ""
            return LyricsResponse(
                lyrics=_lyrics_with_translation(str(lrc), str(translated))
            )
        except Exception:
...
async def _enqueue_qqmusic_song(
    *,
    ...
    try:
        ...
        # Prefer the cover already resolved by the caller. Album MID is a fallback
        # for older clients that do not send cover_url.
        album_cover_url = resolve_qqmusic_cover_url(
            cover_url=cover_url,
            album_mid=album_mid,
            album_cover_resolver=qqmusic.get_song_cover_image,
        )
                ...
                try:
                    ...
                    if kind == "playback":
                        ...
                        if ty == 2:
                            ...
                            try:
                                await ws_manager.broadcast({"type": "finished"})
                            except Exception:
                                pass
                        if ty == 3:
    ...
    *,
    ...
    if track_id.startswith("qqmusic:"):
        ...
        item_id, trial = await _enqueue_qqmusic_song(
            ...
            album_mid="",
            duration_ms=hist_item.duration,
            cover_url=str(hist_item.cover_url or ""),
        )
        return {
            "ok": True,
            "source": "qqmusic",
...
# ---------------------------------------------------------------------------
# WebSocket endpoint — pushes playback state to web clients in real time
# ---------------------------------------------------------------------------

async def _ws_broadcast_position_loop() -> None:
    """Broadcast playback position to all WebSocket clients every ~1 second."""
    import asyncio
    while True:
        ...
        if not ws_manager.has_connections:
            continue
        try:
            st = await voice_status()
            await ws_manager.broadcast(build_ws_progress_payload(st))
        except Exception:
            pass  # voice not available yet, skip a tick


_ws_position_task: asyncio.Task | None = None


@app.websocket("/ws/status")
async def ws_status(websocket: WebSocket) -> None:
    global _ws_position_task
    tokens = settings.get_api_tokens()
    if settings.require_api_auth or tokens:
        if not tokens:
            await websocket.close(code=1013, reason="api token authentication is not configured")
            return
        provided = _get_websocket_api_token(websocket)
        if not provided or not any(
            hmac.compare_digest(provided, token) for token in tokens
        ):
    ...
    # Start the position broadcast loop if not already running
    if _ws_position_task is None or _ws_position_task.done():
        _ws_position_task = asyncio.create_task(_ws_broadcast_position_loop())
    try:
        while True:
...
@app.get("/visual/beat-cache", tags=["Visual API"])
def visual_beat_cache_get(name: str, session: Session = Depends(get_session)) -> dict:
    try:
        _, display_name = normalize_song_name(name)
    except ValueError as error:
        raise HTTPException(status_code=400, detail=str(error)) from error
    row, payload = get_cached_analysis(session, display_name)
    if row is None or payload is None:
        return {"hit": False, "name": display_name}
    return {
        "hit": True,
        "name": row.song_name,
...
@app.post("/visual/beat-cache", tags=["Visual API"])
def visual_beat_cache_store(
    req: BeatAnalysisCacheRequest,
    session: Session = Depends(get_session),
) -> dict:
    try:
        row, payload, created = store_cached_analysis(session, req.name, req.result)
    except ValueError as error:
        raise HTTPException(status_code=400, detail=str(error)) from error
    return {
        "hit": True,
...
@app.delete("/admin/qqmusic/cookie")
async def admin_qqmusic_clear_cookie(
    request: Request,
    session: Session = Depends(get_session),
) -> dict:
    _require_admin_token(request)
    ...
    if row is not None:
        ...
        session.commit()
    qqmusic.set_cookie("")
    return {"ok": True, "admin_cookie_set": False}
...
async def admin_qqmusic_qr_confirm(
    ...
    if c:
        _set_secret(session, "qqmusic_cookie", c)
        logger.info("QQ Music QR login cookie saved")
    else:
        logger.warning("QQ Music QR login completed without a cookie")
