...
def _decode_qq_lyric_text(value: object) -> str:
    """Normalize QQ lyric fields, which may be plain LRC or Base64 text."""
    raw = unescape(str(value or "").strip())
    if not raw:
        return ""

    compact = "".join(raw.split())
    if not raw.lstrip().startswith("[") and len(compact) % 4 == 0:
        try:
...
class QQMusicClient:
    ...
    def set_cookie(self, cookie: str | dict[str, str]) -> None:
        ...
        logger.debug(
            "QQ Music cookie parsed: uin_present=%s, vip_fields_present=%s",
            bool(self._uin),
            bool(found_vip_cookies),
        )
    ...
    async def get_song_lyric(self, songmid: str) -> dict[str, Any]:
        ...
        try:
            ...
            block = (response or {}).get("lyric") or {}
            data = block.get("data") or {}
            lyric = _decode_qq_lyric_text(data.get("lyric"))
            if block.get("code") == 0 and lyric:
                return {
                    **data,
                    "lyric": lyric,
                    "trans": _decode_qq_lyric_text(data.get("trans")),
                }
        except Exception as exc:
            logger.warning(
                "QQ Music primary lyric request failed (%s); using legacy endpoint",
                type(exc).__name__,
            )

        encoded_mid = quote(str(songmid).strip(), safe="")
        login_uin = quote(self._uin or "0", safe="")
        url = (
            "https://c.y.qq.com/lyric/fcgi-bin/fcg_query_lyric_new.fcg"
            f"?songmid={encoded_mid}&songtype=0&format=json&nobase64=1"
            f"&g_tk=5381&loginUin={login_uin}&hostUin=0&inCharset=utf8"
        ...
        return {
            **(legacy or {}),
            "lyric": _decode_qq_lyric_text((legacy or {}).get("lyric")),
            "trans": _decode_qq_lyric_text(
                (legacy or {}).get("trans") or (legacy or {}).get("tlyric")
            ),
        }
    ...
    async def get_music_url_simple(self, songmid: str, quality: str = "320") -> str | None:
        ...
        try:
            ...
            if sip and midurlinfo:
                ...
                logger.debug(
                    "QQ Music returned no playable URL for %s: result=%s",
                    songmid,
                    result_code,
                )
            ...
            logger.debug(
                "QQ Music returned incomplete play URL data for %s: sip_empty=%s, info_empty=%s",
                songmid,
                not sip,
                not midurlinfo,
            )
            ...
            logger.warning(
                "Failed to resolve QQ Music URL for %s (%s)",
                songmid,
                type(e).__name__,
            )
            return None
    ...
    async def get_qr_key(self) -> dict[str, Any]:
        ...
        try:
            logger.debug("Initializing QQ Music login session")
            ...
            async with httpx.AsyncClient(timeout=15, headers=headers, follow_redirects=True) as client:
                ...
                logger.debug("QQ Music XLogin response status: %s", xlogin_response.status_code)
                ...
                logger.debug("Requesting QQ Music QR code")
                qr_response = await client.get(qr_url, params=qr_params)
                logger.debug("QQ Music QR response status: %s", qr_response.status_code)
            ...
            logger.warning("QQ Music QR generation failed (%s)", type(e).__name__)
    ...
    async def confirm_qr_login(self, auth_url: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.debug("Fetched QQ Music cookies after login")
        except Exception as e:
            logger.warning(
                "Failed to fetch QQ Music cookies after login (%s)",
                type(e).__name__,
            )
    ...
    async def check_qr_status(self, qr_key: str, ptqrtoken: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning("QQ Music QR status check failed (%s)", type(e).__name__)
    ...
    async def _fetch_qqmusic_cookies(self) -> None:
        ...
        async with httpx.AsyncClient(timeout=15, headers=headers, follow_redirects=True) as client:
            for url in urls_to_visit:
                try:
                    logger.debug("Visiting QQ Music site to refresh login cookies")
                    r = await client.get(url)
                    self._merge_response_cookies_into_session(r.cookies)
                    ...
                    if music_cookies:
                        logger.debug(
                            "Found %s QQ Music cookie fields while refreshing login",
                            len(music_cookies),
                        )
                    
                except Exception as e:
                    logger.debug(
                        "Failed to refresh QQ Music cookies from %s (%s)",
                        url,
                        type(e).__name__,
                    )
                    continue
        ...
        if final_cookie:
            self.set_cookie(final_cookie)
            logger.debug("Updated QQ Music cookie after login refresh")
