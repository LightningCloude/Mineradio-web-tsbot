...
离线节拍分析完全在浏览器执行，结果仅保存在该用户浏览器的 IndexedDB 中；不会上传到 API、Web 服务或 SQLite。缓存优先使用音乐来源的稳定歌曲 ID，缺失时才使用歌曲名和歌手信息，并会在算法版本变化、过期或浏览器存储不可用时安全重新分析。隐私模式或禁用 IndexedDB 的浏览器会降级为当前标签页的内存缓存。

该功能没有对应的服务端 `/visual/beat-cache` 接口。
