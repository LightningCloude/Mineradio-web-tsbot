...
test('initial status and playback lifecycle activate real song analysis safely', async () => {
  ...
  assert.match(main, /await api\.getBeatAnalysis\(songName\)/);
  assert.match(main, /await api\.storeBeatAnalysis\(songName, result\)/);
  assert.ok(
    main.indexOf('await api.getBeatAnalysis(songName)')
      < main.indexOf('offlineBeatAnalyzer.analyze('),
  );
  assert.match(main, /stored\?\.created === false/);
