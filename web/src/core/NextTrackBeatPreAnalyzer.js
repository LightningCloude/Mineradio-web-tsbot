 ...
 * It never activates a beat grid; completed work is kept in this browser's
 * local cache for the next playback transition.
 */
export class NextTrackBeatPreAnalyzer {
  ...
  _targetFor(song) {
    ...
    return {
      ...
      key,
      name,
      completedKey: this.cache.keyFor(song),
    };
  }
  ...
  async _run(target, generation) {
    ...
    try {
      try {
        const cached = await this.cache.get(target.song);
        if (cached?.beats?.length) {
          this._completed.add(target.completedKey);
          this.onReady({ key: target.key, name: target.name, source: 'local-cache' });
          this.onStatus({ key: target.key, name: target.name, status: 'ready', source: 'local-cache' });
          ready = true;
          this.logger.log('[MineraTS] Next-track local beat cache hit:', target.name);
          return;
        }
      } catch (error) {
        this.logger.warn('[MineraTS] Next-track local cache lookup failed:', error?.message || error);
      }
      ...
      // still valid for this browser's future playback.
      this._completed.add(target.completedKey);
      ...
      ready = true;
      try {
        await this.cache.set(target.song, result);
      } catch (error) {
        this.logger.warn('[MineraTS] Next-track local cache write failed:', error?.message || error);
      }
    } catch (error) {
