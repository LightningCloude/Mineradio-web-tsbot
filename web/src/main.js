...
async function _tryAnalyzeSongOffline(song, trackId) {
  ...
  const songName = String(song?.title || song?.name || '').trim();
  let analysisReady = false;
  _emitBeatAnalysisStatus(song, trackId, 'analyzing');
  if (songName) {
    try {
      const cached = await api.getBeatAnalysis(songName);
      if (cached?.hit && cached.result?.beats?.length) {
        analysisReady = true;
        _emitBeatAnalysisStatus(song, trackId, 'ready', 'shared-cache');
        if (_activateBeatAnalysis(trackId, cached.result, 'Shared cached')) return true;
      }
      ...
      // A cache outage must never block local analysis or playback visuals.
      console.warn('[MineraTS] Shared beat cache lookup failed:', error?.message || error);
    }
  }
  ...
  try {
    ...
    if (result?.beats?.length && songName) {
      try {
        const stored = await api.storeBeatAnalysis(songName, result);
        // If another user won the unique-key insert, converge on that first
        // shared result so every connected client uses the same beat grid.
        if (stored?.created === false && stored.result) {
          _activateBeatAnalysis(trackId, stored.result, 'Shared canonical');
        }
      } catch (error) {
        console.warn('[MineraTS] Shared beat cache upload failed:', error?.message || error);
      }
    }
    if (!analysisReady) _emitBeatAnalysisStatus(song, trackId, 'idle');
    return loaded;
